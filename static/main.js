...
function speak(string) {
	...
	// const vietnameseVoice = allVoices.find(voice => voice.lang.includes("vi"));
	u.voice = allVoices.filter(voice => voice.name === "Alex")[0];// vietnameseVoice;
	u.text = string;
	u.lang = "en-US"; // "vi-VN";
...
$('#send_button').on('click', async function (e) {
	...
	// Create message input from user
	const messages = $('#msg_input').val() ;
	...
	// show bot message
	setTimeout(function () {
		showBotMessage(response.answer); //randomstring()
